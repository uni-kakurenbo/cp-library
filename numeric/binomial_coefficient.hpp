...
template<class T, class R = T, class Montgomery = arbitrary_montgomery_32bit>
...
struct binomial_coefficient_prime_power_mod {
  ...
  private:
    ...
    void _init() {
        ...
        this->_one = this->_montgomery_m.convert_raw(1);
        ...
        REP(i, 2, size) {
            if(this->_barrett_p.reduce(i) == 0) {
                ...
                this->_fact[i + 1] = this->_montgomery_m.multiply(this->_fact[i - 1], this->_montgomery_m.convert_raw(this->_barrett_m.reduce(i + 1)));
                ++i;
            }
            else {
                this->_fact[i] = this->_montgomery_m.multiply(this->_fact[i - 1], this->_montgomery_m.convert_raw(this->_barrett_m.reduce(i)));
            }
        }

        this->_inv_fact[size - 1] = this->_montgomery_m.pow(this->_fact[size - 1], this->_m / this->_p * (this->_p - 1) - 1, this->_one);

        REPD(i, 2, size - 1) {
            this->_inv_fact[i] = this->_montgomery_m.multiply(this->_inv_fact[i + 1], this->_montgomery_m.convert_raw(this->_barrett_m.reduce(i + 1)));
            if(this->_barrett_p.reduce(i) == 0) {
  ...
  public:
    ...
    mod_type lucus(value_type n, value_type k) const noexcept(NO_EXCEPT) {
        ...
        while(n > 0) {
            ...
            res = this->_montgomery_m.multiply(res, this->_fact[n0]);
            res = this->_montgomery_m.multiply(res, this->_montgomery_m.multiply(this->_inv_fact[n0 - k0], this->_inv_fact[k0]));
        }

        return static_cast<mod_type>(this->_montgomery_m.reduce(res));
    }
    ...
    mod_type comb(value_type n, value_type k) const noexcept(NO_EXCEPT) {
        ...
        while(n > 0) {
            res = this->_montgomery_m.multiply(res, this->_fact[this->_barrett_m.reduce(n)]);
            res = this->_montgomery_m.multiply(res, this->_inv_fact[this->_barrett_m.reduce(k)]);
            res = this->_montgomery_m.multiply(res, this->_inv_fact[this->_barrett_m.reduce(r)]);
        ...
        if(eq & 1) res = this->_montgomery_m.multiply(res, this->_delta);
        res = this->_montgomery_m.multiply(res, this->_montgomery_m.pow(this->_montgomery_m.convert_raw(this->_p), e0, this->_one));
...
struct binomial_coefficient {
  ...
  private:
    ...
    internal_binomial<arbitrary_montgomery_32bit> _internal_2p;
    ...
    void _init() noexcept(NO_EXCEPT) {
        ...
        for(u32 i=2; i*i<=md; ++i) {
            if(md % i == 0) {
                ...
                if(i == 2) {
                    this->_internal_2p = internal_binomial<arbitrary_montgomery_32bit>(2, j, this->_max);
        ...
        if(md != 1) {
            ...
            if(md == 2) this->_internal_2p = internal_binomial<arbitrary_montgomery_32bit>(2, 1, this->_max);
