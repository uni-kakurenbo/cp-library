...
set +e
{
    ...
    DEPENDENCIES="$(time g++-14 -std=gnu++20 -MM -I"${WORKING_DIRECTORY}" "${TARGET}" | sed -E s/^.*\.o:\ //)"
    ...
    if [ "${LAST_MODIFIED_AT}" -gt "${LAST_VERIFIED_AT}" ]; then
        time g++-14 "${OPTIONS[@]}" -O0 -I"${WORKING_DIRECTORY}" -I"${WORKING_DIRECTORY}/../ac-library" "${TARGET}" 2>".warn-${PID}.txt"
