...
template<class I>
std::string join(const I first, const I last, const std::string& sep = "") noexcept(DEV_ENV) {
...
template<class V>
std::string join(V& v, const std::string& sep = "") noexcept(DEV_ENV) {
    return join(std::begin(v), std::end(v), sep);
}


template<class I, class T = typename std::iterator_traits<I>::value_type>
T sum(const I first, const I second, const T& base = 0) noexcept(DEV_ENV) {
    return std::accumulate(first, second, base);
}

template<class V, class T = typename V::value_type>
auto sum(V& v, T base = 0) noexcept(DEV_ENV) {
