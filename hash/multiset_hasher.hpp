...
struct multiset_hasher {
  ...
  protected:
    ...
    static inline hash_type _id(const T& v) {
    ...
    inline void _add_hash(const hash_type h, const hash_type count) {
        this->_hash += multiset_hasher::mul(h, count);
        if(this->_hash >= multiset_hasher::mod) this->_hash -= multiset_hasher::mod;
    }
    inline void _remove_hash(const hash_type h, const hash_type count) {
  ...
  public:
    ...
    template<class I> multiset_hasher(const I first, const I last) : multiset_hasher() {
    ...
    inline size_type empty() const { return this->get() == 0; }
    inline void clear() const { this->_hash = 0; }
    ...
    inline void erase(const T& v, const size_type count = 1) {
