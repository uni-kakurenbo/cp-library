...
struct range_set_range_min : implicit_treap<T0, T1> {
    using size_t = typename implicit_treap<T0, T1>::size_t;

    template<class... Args>
    explicit range_set_range_min(Args&&... args)
    : implicit_treap<T0, T1>::implicit_treap(std::numeric_limits<T0>::max(), std::numeric_limits<T1>::min()) {
