...
struct RangeSetRangeMin : ImplicitTreap<T0, T1> {
    using Size = typename ImplicitTreap<T0, T1>::Size;

    template<class... Args>
    explicit RangeSetRangeMin(Args&&... args)
    : ImplicitTreap<T0, T1>::ImplicitTreap(std::numeric_limits<T0>::max(), std::numeric_limits<T1>::min()) {
