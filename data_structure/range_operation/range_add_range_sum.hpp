...
struct range_add_range_sum : implicit_treap<T0, T1> {
    using size_t = typename implicit_treap<T0, T1>::size_t;

    template<class... Args>
    explicit range_add_range_sum(Args&&... args) : implicit_treap<T0, T1>::implicit_treap(0, 0) {
    ...
    inline T1 p(const T1 x, const size_t len) const override { return x * len; }
