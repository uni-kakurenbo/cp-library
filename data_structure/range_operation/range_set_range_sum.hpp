...
struct range_set_range_sum : implicit_treap<T0, T1> {
    using size_t = typename implicit_treap<T0, T1>::size_t;

    template<class... Args>
    explicit range_set_range_sum(Args&&... args) : implicit_treap<T0, T1>::implicit_treap(0, std::numeric_limits<T1>::min()) {
    ...
    inline T1 p(const T1 x, const size_t len) const override { return x == std::numeric_limits<T1>::min() ? std::numeric_limits<T1>::min() : x * len; }
