...
struct Base : private Uncopyable {
  ...
  private:
    ...
    inline void split(const Tree tree, const Size key, Tree& left, Tree& right) const {
        if(!tree) {
            left = right = nullptr;
            return;
        }
        this->pushdown(tree);
        const Size implicit_key = this->cnt(tree->left) + 1;
        if(key < implicit_key) {
    ...
    inline void erase(Tree& tree, const Size key) const {
        Tree t1, t2, t3;
    ...
    inline T0 prod(Tree tree, const Size first, const Size last) const {
    ...
    inline void reverse(Tree tree, const Size first, const Size last) const {
        if(first > last) return;
        Tree t1, t2, t3;
        this->split(tree, first, t1, t2);
        this->split(t2, last - first, t2, t3);
    ...
    inline void rotate(Tree tree, const Size first, const Size middle, const Size last) const {
  ...
  public:
    inline Size size() const { return this->cnt(this->root); }
  ...
  protected:
    ...
    inline T0 prod(const Size first, const Size last) const { return this->prod(this->root, first, last); }

    inline void erase(const Size pos) { this->erase(this->root, pos); }

    inline void reverse(const Size first, const Size last) const { this->reverse(this->root, first, last); }

    inline void rotate(const Size first, const Size middle, const Size last) const { this->rotate(this->root, first, middle, last); }

    // 二分探索。[first, last)内のkでf0(tr[k], value) != xとなる最左/最右のもの。存在しない場合は-1
    // たとえばMinMonoidの場合、x未満の最左/最右の要素の位置を返す
    inline Size find(const Size first, const Size last, const T0& value, const bool dir_left) const {
        if(first >= last) return -1;
        Tree t1, t2, t3;
        this->split(this->root, first, t1, t2);
        this->split(t2, last - first, t2, t3);
        const Size ret = this->find(t2, value, first, dir_left);
        this->merge(t2, t2, t3);
...
template<class T0, class T1>
struct ImplicitTreap : ImplicitTreapLib::Base<T0,T1> {
  private:
    using Base = ImplicitTreapLib::Base<T0,T1>;

  public:
    using Size = typename Base::Size;

  protected:
    inline void _validate_index_in_right_open(const Size _pos) const {
        dev_assert(0 <= _pos and _pos < this->size());
    }
    inline void _validate_index_in_closed(const Size _pos) const {
        dev_assert(0 <= _pos and _pos <= this->size());
    }
    inline void _validate_rigth_open_interval(const Size _l, const Size _r) const {
        dev_assert(0 <= _l and _l <= _r and _r <= this->size());
    }

    inline Size _positivize_index(const Size pos) const {
        return pos < 0 ? this->size() + pos : pos;
    }

  public:
    explicit ImplicitTreap(const T0 u0, const T1 u1) : Base::Base(u0, u1) {}
  ...
  public:
    ...
    inline void insert(Size pos, const I first, const I last) {
        for(auto itr=first; itr != last; ++itr, ++pos) {
            this->insert(pos, *itr);
        }
    }

    inline void insert(Size pos, const T0& value) {
        pos = this->_positivize_index(pos);
        this->_validate_index_in_closed(pos);
        this->Base::insert(pos, value);
    }

    inline void insert(const Size first, const T0& value, const Size count) {
        REP(pos, count) this->insert(first + pos, value);
    }

    inline void push_front(const T0& value, const Size count = 1) { this->insert(0, value, count); }
    inline void push_back(const T0& value, const Size count = 1) { this->insert(this->size(), value, count); }

    inline void resize(const Size size, const T0& value = T0{}) {
        REP(this->size() - size) this->erase(-1);
        REP(i, this->size(), size) this->push_back(value);
    }

    inline void assign(const Size size, const T0& value = T0{}) {
    ...
    inline void fill(const T0& value) {
        const Size count = this->size();
        this->clear(), this->insert(0, value, count);
    }
    inline void fill(const Size first, const Size last, const T0& value) {
        REP(pos, first, last) this->erase(pos), this->insert(pos, value);
    }

    inline void erase(Size pos) {
        pos = this->_positivize_index(pos);
        this->_validate_index_in_right_open(pos);
        this->Base::erase(pos);
    }
    inline void erase(const Size pos, const Size count) { REP(count) this->erase(pos); }
    inline void clear() { this->erase(0, this->size()); }

    inline void pop_front(const Size count = 1) { this->erase(0, count); }
    inline void pop_back(const Size count = 1) { this->erase(this->size() - count, count); }


    inline void apply(Size first, Size last, const T1& value) {
    ...
    inline void apply(const Size p, const T1& value) { this->apply(p, p+1, value); }
    inline void apply(const T1& value) { this->apply(0, this->size(), value); }


    inline T0 get(Size pos) const {
        pos = this->_positivize_index(pos);
        this->_validate_index_in_right_open(pos);
        return this->Base::prod(pos, pos+1);
    }
    inline T0 operator[](const Size pos) const { return this->get(pos); }

    inline T0 front() const { return this->get(0); }
    inline T0 back() const { return this->get(this->size()-1); }

    inline T0 prod(Size first, Size last) const {
        first = this->_positivize_index(first), last = this->_positivize_index(last);
        this->_validate_rigth_open_interval(first, last);
    ...
    inline void reverse(Size first, Size last) const {
        first = this->_positivize_index(first), last = this->_positivize_index(last);
        this->_validate_rigth_open_interval(first, last);
    ...
    inline void rotate(Size first, Size middle, Size last) const {
    ...
    inline void rotate(const Size middle) const { this->rotate(0, middle, this->size()); }

    inline Size find(Size first, Size last, const T0& value, const bool dir_left = true) const {
        first = this->_positivize_index(first), last = this->_positivize_index(last);
    ...
    inline Size find(const T0& value, const bool dir_left = true) const {
        return this->find(0, this->size(), value, dir_left);
    }


    struct Iterator : virtual Internal::IContainerIterator<T0,ImplicitTreap> {
        Iterator(const ImplicitTreap *const ref, const Size pos) : Internal::IContainerIterator<T0,ImplicitTreap>(ref, pos) {}
