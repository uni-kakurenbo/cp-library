...
struct base : private uncopyable {
  ...
  private:
    ...
    inline void erase(Tree& tree, const size_type pos) const noexcept(NO_EXCEPT) {
        ...
        this->split(tree, pos + 1, t0, t1);
        this->split(t0, pos, t0, t2);
        this->merge(tree, t0, t1);
    ...
    template<bool LEFT>
    inline size_type find(const Tree tree, operand& v, const size_type offset) const noexcept(NO_EXCEPT) {
        if(tree->acc + v == v) {
            return -1;
        }
        ...
        if constexpr(LEFT) {
            if(tree->left and tree->left->acc + v != v) {
                return this->find<true>(tree->left, v, offset);
            }
            else {
                return tree->v + v != v ? offset + this->cnt(tree->left) : this->find<true>(tree->right, v, offset + this->cnt(tree->left) + 1);
            }
        }
        else {
            if(tree->right and tree->right->acc + v != v) {
                return this->find<false>(tree->right, v, offset + this->cnt(tree->left) + 1);
            }
            else {
                return tree->v + v != v ? offset + this->cnt(tree->left) : this->find<false>(tree->left, v, offset);
            }
        }
    }
  ...
  public:
    ...
    template<bool LEFT>
    inline size_type find(const size_type l, const size_type r, const operand& v) const noexcept(NO_EXCEPT) {
        if(l == r) return -1;
...
struct sequency_core : base<typename Action::operand, typename Action::operation, Action::map, Action::fold> {
  public:
    using action = Action;
  ...
  public:
    template<class... Args>
    explicit sequency_core(Args&&... args) noexcept(NO_EXCEPT) : sequency_core() { this->assign(std::forward<Args>(args)...); }

    template<std::convertible_to<value_type> T>
    sequency_core(const std::initializer_list<T>& values) noexcept(NO_EXCEPT)
      : sequency_core(std::ranges::begin(values), std::ranges::end(values))
    {}

    sequency_core() noexcept(NO_EXCEPT) {}
    ...
    struct point_reference : internal::point_reference<sequency_core> {
    ...
    struct range_reference : internal::range_reference<sequency_core> {
        ...
        inline size_type find(const value_type& v) const noexcept(NO_EXCEPT) {
            return this->_super->template base::find<LEFT>(this->_begin, this->_end, v);
        }
    };
    ...
    // If no such k is found, return -1.
    template<bool LEFT = true>
    inline size_type find(size_type l, size_type r, const value_type& v) const noexcept(NO_EXCEPT) {
        l = this->_positivize_index(l), r = this->_positivize_index(r);
        assert(0 <= l && l <= r && r <= this->size());
        return this->template base::find<LEFT>(l, r, v);
    }
    ...
    template<bool LEFT = true>
    inline size_type find(const value_type& v) const noexcept(NO_EXCEPT) {
        return this->find<LEFT>(0, this->size(), v);
    }

    struct iterator;

  protected:
    using iterator_interface = internal::container_iterator_interface<value_type,sequency_core,iterator>;

  public:
    struct iterator : iterator_interface {
        iterator() noexcept = default;
        iterator(const sequency_core *const ref, const size_type p) noexcept(NO_EXCEPT) : iterator_interface(ref, p) {}
...
template<actions::internal::full_action Action>
    requires internal::available_with<internal::implicit_treap_impl::sequency_core, Action>
struct implicit_treap : internal::implicit_treap_impl::sequency_core<Action> {
    using internal::implicit_treap_impl::sequency_core<Action>::sequency_core;
};
