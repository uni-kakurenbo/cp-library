...
struct base {
  ...
  protected:
    ...
    explicit base(const size_type n) noexcept(DEV_ENV)
    ...
    inline S* data() noexcept(DEV_ENV) { return this->_data; }
    inline const S* data() const noexcept(DEV_ENV) { return this->_data; }

    inline void apply(size_type p, const S& x) noexcept(DEV_ENV) {
        for(p++; p<=this->_n; p += p & -p) this->_data[p-1] = this->_data[p-1] + x;
    }

    inline void set(const size_type p, const S& x) noexcept(DEV_ENV) {
        assert(this->get(p) == this->fold(p, p+1));
        this->apply(p, x + -this->get(p));
    }

    inline S fold(size_type r) const noexcept(DEV_ENV) {
        S s = S{};
    ...
    inline S get(size_type p) const noexcept(DEV_ENV) { return this->fold(p, p+1); }
...
struct core<monoid,std::void_t<typename algebraic::internal::is_monoid_t<monoid>>> : base<monoid> {
  ...
  public:
    ...
    inline bool empty() const noexcept(DEV_ENV) { return this->base::size() == 0; }
    ...
    inline const range_reference operator()(const size_type l, const size_type r) const noexcept(DEV_ENV) { return range_reference(this, l, r); }
