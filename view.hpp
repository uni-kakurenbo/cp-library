...
struct view : internal::view_impl::base {
  ...
  public:
    view(Ref *const ref) : _ref(ref) {}

    inline view& reset(Ref *const ref) { this->_ref = ref; return *this; }

    inline Ref* operator->() const noexcept { return this->_ref; }
    inline Ref& operator*() const noexcept { return *this->_ref; }

    inline size_type size() const { return this->_taken; }

    // inline auto begin() const { return std::next(std::begin(*this->_ref), this->_pos); }
    // inline auto end() const { return std::next(this->begin(), this->_taken); }

    inline view& drop(const size_type pos = 0) { this->_pos = pos; this->_validate(); return *this; }
    inline view& shift(const size_type count = 1) { this->_pos += count; this->_validate(); return *this; };

    inline view& take(const size_type count) { this->_taken = count; this->_validate(); return *this; }
    inline view& take() { this->_taken = std::size(*this->_ref);; return *this; }
  ...
  public:
    struct iterator : virtual iterator_interface {
        iterator(const view *const ref, const size_type pos) : iterator_interface(ref, pos) {}

        inline value_type operator*() const { return this->ref()->operator[](this->pos()); }
...
struct cyclic_view : internal::view_impl::base {
  ...
  public:
    cyclic_view(Ref *const ref) : _ref(ref), _taken(std::size(*ref)) {}

    inline cyclic_view& reset(Ref *const ref) { this->_ref = ref, this->_taken = std::size(*ref); return *this; }
    ...
    inline cyclic_view& drop(const size_type pos = 0) { this->_pos = pos; return *this; }
    inline cyclic_view& shift(const size_type count = 1) { this->_pos += count; return *this; };

    inline cyclic_view& take(const size_type count) { this->_taken = count; return *this; }
    inline cyclic_view& take() { this->_taken = std::size(*this->_ref); return *this; }
  ...
  public:
    struct iterator : virtual iterator_interface {
        iterator(const cyclic_view *const ref, const size_type pos) : iterator_interface(ref, pos) {}
...
struct multi_view : internal::view_impl::base {
  ...
  public:
    multi_view(Ref *const ref) : _ref(ref) {}

    inline multi_view& reset(Ref *const ref) { this->_ref = ref; return *this; }
    ...
    template<class... Pos> inline multi_view& drop(const Pos... pos) { return this->drop({ pos... }); }
    inline multi_view& drop() { std::memset(this->_pos, 0, sizeof(this->_pos)); return *this; }

    inline const multi_view& transpose(const transposer& f) noexcept { this->_transposer = f; return *this; }


    inline auto& operator[](const std::initializer_list<size_type> pos_) noexcept {
        const auto pos = this->_transposer(pos_);
        return this->_access(this->_ref, pos.begin());
    }
    inline const auto& operator[](std::initializer_list<size_type> pos_) const noexcept {
        const auto pos = this->_transposer(pos_);
        return this->_access(this->_ref, pos.begin());
    }

    template<class... Pos> inline auto& operator()(const Pos... pos) noexcept { return this->operator[]({ pos... }); }
...
template<class Ref> struct view_2d : internal::view_impl::base {
  ...
  public:
    view_2d(Ref *const ref) : _ref(ref) {}
    ...
    inline size_type size() const { return std::size(*this->_ref); }
    inline size_type height() const { return std::size(*this->_ref); }
    ...
    template<class... Pos> inline view_2d& drop(const Pos... pos) { return this->drop({ pos... }); }
    inline view_2d& drop() { std::memset(this->_pos, 0, sizeof(this->_pos)); return *this; }

    inline const view_2d& transpose(const transposer& f) noexcept { this->_transposer = f; return *this; }

    inline auto& operator()(const size_type i, const size_type j) noexcept { return this->operator[]({ i, j }); }
    inline const auto& operator()(const size_type i, const size_type j) const noexcept { return this->operator[]({ i, j }); }

    inline auto& operator[](std::pair<size_type,size_type> pos) noexcept {
        pos = this->_transposer(pos);
        return this->_ref->operator[](pos.first)[pos.second];
    }
    inline const auto& operator[](std::pair<size_type,size_type> pos) const noexcept {
        pos = this->_transposer(pos);
        return this->_ref->operator[](pos.first)[pos.second];
    }
};
