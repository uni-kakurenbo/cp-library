...
std::string to_base_n_string(T v) noexcept(NO_EXCEPT) {
...
template <class T>
std::string to_base_n_string(T v, std::size_t b) noexcept(NO_EXCEPT) {
    constexpr char CHARS[] = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
...
template <class T>
std::vector<T> to_base_n_vector(T v, std::size_t b = 2) noexcept(NO_EXCEPT) {
    assert(1 < b);
    assert(0 <= v);
    std::vector<T> res;
    while(v > 0) {
