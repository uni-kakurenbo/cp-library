...
private:
    ...
    uint64_t rangeSum(const uint64_t begin, const uint64_t end, const uint64_t depth, const uint64_t c, const uint64_t x, const uint64_t y) {
        ...
        debug(depth);
        debug(this->cumulative_sum.at(depth));
        ...
        // debug(rank0_begin, rank0_end, rank1_begin, rank1_end);
...
uint64_t solve(int N, int K, V<uint64_t> &A) {
    ...
    for(int i=0; i<N; ++i) { std::cerr << wm.access(i) << " "; }
    std::cerr << "\n";

    std::cerr << wm.rangeSum(0, N, 4, 9) << "\n";
