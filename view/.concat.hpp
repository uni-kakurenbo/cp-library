  ...
  public:
    ...
    explicit concat_view(V0 v0, V1 v1) noexcept(NO_EXCEPT)
      : _v0(std::move(v0)), _v1(std::move(v1)),
        _f0(std::ranges::begin(this->_v0)), _l0(std::ranges::end(this->_v0)),
        _f1(std::ranges::begin(this->_v1)), _l1(std::ranges::end(this->_v1))
    {};
    ...
    inline size_type size() const noexcept(NO_EXCEPT) {
        if(this->_s0 < 0) this->_s0 = std::ranges::distance(this->_f0, this->_l0);
        if(this->_s1 < 0) this->_s1 = std::ranges::distance(this->_f1, this->_l1);
...
struct concat_view<V> : std::ranges::ref_view<V> {
...
struct concat_view<V0,V1> : internal::view_impl::concat_view<V0,V1> {
...
template<class V0, class V1, class... Vs>
    requires std::ranges::view<V0> && std::ranges::view<V1> && (std::ranges::view<Vs> && ...)
struct concat_view<V0,V1,Vs...> : concat_view<concat_view<V0,V1>,Vs...> {
  public:
...
template<class V>
explicit concat_view(V&&) -> concat_view<std::views::all_t<V>>;

template<class V0, class V1>
explicit concat_view(V0&&, V1&&) -> concat_view<std::views::all_t<V0>,std::views::all_t<V1>>;

template<class V0, class V1, class... Vs>
explicit concat_view(V0&&, V1&&, Vs&&...) -> concat_view<std::views::all_t<V0>,std::views::all_t<V1>,std::views::all_t<Vs>...>;
...
template<class... Ts>
concept can_concat_view = requires { concat_view<std::views::all_t<Ts>...>(std::declval<Ts>()...); };
...
struct Concat {
    template<class... Ts> requires(sizeof...(Ts) == 0 || internal::can_concat_view<Ts...>)
